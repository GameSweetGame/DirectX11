...
C언어 수업을 들으며 제작한 콘솔 기반 액션 게임입니다.
...
화면 출력은 더블 버퍼링(`DrawToBackBuffer`)을 사용해 콘솔 깜빡임을 줄였습니다.

### 렌더링

콘솔 창에 특수문자를 출력하고 지우는 방식으로 렌더링합니다.
이동 시 이전 위치의 문자를 지우고, 새 좌표에 다시 출력합니다.
`SetCurrentCursorPos`로 커서를 원하는 위치로 직접 이동시켜 출력합니다.
